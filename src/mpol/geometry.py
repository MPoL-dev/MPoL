...
def flat_to_observer(x, y, omega=None, incl=None, Omega=None):
    """Apply the rotation matrices to go from flat (x,y,z) to observer frame (X,Y,Z).

    It is assumed that the +Z axis points *towards* the observer. It is assumed that the model is flat in the (x,y) frame, and so the operations involving ``z`` are neglected. But the model lives in 3D Cartesian space. 
...
def observer_to_flat(X, Y, omega=None, incl=None, Omega=None):
    """Apply the rotation matrices to go from the observer frame (X,Y,Z) to the flat (x,y,z) frame.
