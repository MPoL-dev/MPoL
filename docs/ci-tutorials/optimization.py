...
First we'll install the package. You'll probably also want to try running this code with the GPU acceleration as well. To do that, go to `Runtime > Change Runtime Type` and then select the GPU as the hardware acceleration.
"""
...
"""## Data
The fundamental dataset is the set of complex-valued visibility measurements: lists of the $u,v$ coordinates, real and imaginary visibility values ($\Re$ and $\Im$), and visibility weights ($w \propto 1/\sigma^2$). If you have a CASA measurement set, you’ll want to export these quantities. You can achieve this by writing a CASA script yourself or using the [UVHDF5](https://github.com/AstroChem/UVHDF5) package. It goes without saying that you should make sure the data are correctly calibrated, particularly the [weights](https://casaguides.nrao.edu/index.php/DataWeightsAndCombination).

One important thing to note is that the effective CASA (and AIPS) baseline convention is opposite that of the interferometric coordinate system typically presented in textbooks, e.g., [TMS](https://ui.adsabs.harvard.edu/abs/2017isra.book.....T/abstract), Fig. 3.2. Urvashi Rao has a very helpful [memo](https://casa.nrao.edu/casadocs/casa-5.6.0/memo-series/casa-memos/casa_memo2_coordconvention_rau.pdf) explaining the baseline convention in CASA. Essentially, this means you’ll want to take the complex conjugate of your visibilities to have your images show up in the correct orientation.

To test out the package, you can download a fake dataset of Saturn here
"""

!wget https://zenodo.org/record/3634225/files/data.npz
...
weights = npzfile["weights"] # [1/Jy^2]

"""For convenience, we provide a dataset wrapper for these quantities, `mpol.datasets.UVDataset`. After loading your data, you can initialize this with
...
device = "cuda"
# device = "cpu" # uncomment this if you just want to run on CPU
npix = 800
imwidth = 8.0 # arcsec 
cell_size = imwidth / npix # arcsec/pixel
dataset = UVDataset(uu=uu, vv=vv, data_re=data_re, data_im=data_im, weights=weights, cell_size=cell_size, npix=npix, device=device)
...
The `mpol.images.ImageCube` requires the following options
"""
...
The main functionility of the `ImageCube` class is to forward-model the visibilities starting from an image, done using the `ImageCube.forward` method. This method is called automatically when you use `model()`. To save computation, the core image representation is actually stored pre-fftshifted in the `model._cube` variable, but you can query the de-shifted version using `model.cube`.
...
plt.imshow(cube[0], origin="lower", extent=model.extent, interpolation="none") # plot only the one channel
